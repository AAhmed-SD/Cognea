...
class TestLogAuditEvent:
    ...
    def test_log_audit_event_success(self, mock_logger, mock_get_supabase):
        ...
        with patch('services.audit.datetime') as mock_datetime:
            ...
            # Verify logging
            mock_logger.info.assert_called_once_with(f"Audit log: {action} {resource} {resource_id} by {user_id}")
    ...
    def test_log_audit_event_minimal_data(self, mock_logger, mock_get_supabase):
        ...
        with patch('services.audit.datetime') as mock_datetime:
            ...
            # Verify logging
            mock_logger.info.assert_called_once_with(f"Audit log: {action} {resource} None by None")
    ...
    def test_log_audit_event_supabase_error(self, mock_logger, mock_get_supabase):
        ...
        mock_logger.error.assert_called_once_with("Failed to log audit event: Database connection failed")
...
class TestAuditIntegration:
    ...
    def test_full_audit_flow(self, mock_logger, mock_get_supabase):
        ...
        with patch('services.audit.datetime') as mock_datetime:
            ...
            # Verify logging
            mock_logger.info.assert_called_once_with(f"Audit log: {action} {resource} None by {user_id}")
