...
class TestGetSupabaseServiceClient:
    ...
    def test_get_supabase_service_client_success(self, mock_create_client):
        ...
        assert client is not None
        assert isinstance(client, Client)
        mock_create_client.assert_called_once_with('https://test.supabase.co', 'test_service_key')
    ...
    def test_get_supabase_service_client_no_key(self):
        ...
        with pytest.raises(ValueError, match="SUPABASE_SERVICE_ROLE_KEY environment variable must be set for admin operations"):
            get_supabase_service_client()
...
class TestTestConnection:
    ...
    def test_test_connection_failure(self, mock_client):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_once_with("❌ Supabase connection failed: Connection failed")
    ...
    def test_test_connection_network_error(self, mock_client):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert result is False
            mock_print.assert_called_once_with("❌ Supabase connection failed: Network unreachable")
    ...
    def test_test_connection_auth_error(self, mock_client):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert result is False
            mock_print.assert_called_once_with("❌ Supabase connection failed: Invalid API key")
    ...
    def test_connection_test_with_specific_exception(self, mock_client):
        ...
        for exception, expected_message in test_cases:
            ...
            with patch('builtins.print') as mock_print:
                ...
                assert result is False
                mock_print.assert_called_once_with(f"❌ Supabase connection failed: {expected_message}")
