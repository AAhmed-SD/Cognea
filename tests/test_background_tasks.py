...
class TestCategorizeTaskError:
    ...
    def test_timeout_error_categorization(self):
        ...
        assert result["type"] == TaskErrorType.NETWORK_ERROR  # TimeoutError is a subclass of OSError
        assert result["retryable"] is True
        assert result["retry_delay_multiplier"] == 2.0
        assert result["max_retries"] == 5
...
class TestLogBackgroundTask:
    ...
    def test_log_background_task_success(self, mock_uuid, mock_datetime, mock_metrics, mock_logger):
    ...
    @patch('services.background_tasks.datetime')
    @patch('services.background_tasks.uuid')
    def test_log_background_task_failure(self, mock_uuid, mock_datetime, mock_metrics, mock_logger):
