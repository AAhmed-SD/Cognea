...
class TestTasksEndpoints:
    ...
    async def test_update_task_unauthorized(self, async_client):
        ...
        task_id = str(uuid4())
        response = await async_client.put(f"/api/tasks/{task_id}", json={"title": "Updated Task"})
        assert response.status_code in [401, 403]
...
class TestGoalsEndpoints:
    ...
    async def test_update_goal_unauthorized(self, async_client):
        ...
        goal_id = str(uuid4())
        response = await async_client.put(f"/api/goals/{goal_id}", json={"title": "Updated Goal"})
        assert response.status_code in [401, 403]
...
class TestScheduleBlocksEndpoints:
    ...
    async def test_create_schedule_block_unauthorized(self, async_client):
        ...
        response = await async_client.post("/api/schedule-blocks/", json=test_schedule_block)
        assert response.status_code in [401, 403]
...
class TestFlashcardsEndpoints:
    ...
    async def test_review_flashcard_unauthorized(self, async_client):
        ...
        response = await async_client.post(f"/api/flashcards/{flashcard_id}/review", params={"quality": 5})
        assert response.status_code in [401, 403]
...
class TestAIEndpoints:
    ...
    async def test_plan_day_unauthorized(self, async_client):
        ...
        response = await async_client.post("/api/plan-day", json={
            "date": "2024-01-15",
    ...
    async def test_generate_flashcards_unauthorized(self, async_client):
        ...
        response = await async_client.post("/api/generate-flashcards", json={
            "topic": "Python",
            "difficulty": "beginner",
            "count": 5
        })
...
class TestSecurity:
    ...
    async def test_cors_headers(self, async_client):
        ...
        response = await async_client.options("/", headers={"Origin": "http://localhost:3000"})
        headers = response.headers
