...
class TestNotionFlashcardGenerator:
    ...
    pass
        ...
        flashcards = generator._parse_ai_response(
            ai_response, "test_page_id", "Test Page"
        )
...
async def test_notion_integration_end_to_end(
    ...
    with patch(
        "services.notion.sync_manager.get_supabase_client", return_value=mock_supabase
    ):
        # Initialize services
        flashcard_generator = NotionFlashcardGenerator(
            mock_notion_client, mock_openai_service
        ...
        flashcards = await flashcard_generator.generate_flashcards_from_page(
            page_id="test_page_id", count=1, difficulty="easy"
        )
