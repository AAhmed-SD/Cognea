...
async def calendar_connect(user_id: str):
    ...
    data = {"user_id": user_id, "connected": True, "connected_at": datetime.now().isoformat()}
    result = supabase.table("calendar_connections").upsert(data, on_conflict=["user_id"]).execute()
    return {"user_id": user_id, "status": "calendar connected", "connected_at": data["connected_at"]}
