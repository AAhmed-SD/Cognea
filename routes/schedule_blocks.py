...
from models.schedule_block import ScheduleBlock, ScheduleBlockCreate, ScheduleBlockUpdate
...
async def create_schedule_block(
    schedule_block: ScheduleBlockCreate,
...
@router.get("/", response_model=List[ScheduleBlock], summary="Get schedule blocks for user")
async def get_schedule_blocks(
    current_user: dict = Depends(get_current_user),
    start_date: Optional[datetime] = Query(None, description="Filter blocks starting from this date"),
    end_date: Optional[datetime] = Query(None, description="Filter blocks ending before this date"),
    context: Optional[str] = Query(None, description="Filter by context (e.g., 'Work', 'Study')"),
    ...
    try:
        ...
        query = supabase.table("schedule_blocks").select("*").eq("user_id", current_user["id"])
...
@router.get("/today", response_model=List[ScheduleBlock], summary="Get today's schedule blocks")
async def get_today_schedule_blocks(
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        supabase = get_supabase_client()
        result = supabase.table("schedule_blocks").select("*").eq("user_id", current_user["id"]).gte("start_time", start_of_day.isoformat()).lte("end_time", end_of_day.isoformat()).order("start_time", desc=False).execute()
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch today's schedule blocks")

@router.get("/{block_id}", response_model=ScheduleBlock, summary="Get a specific schedule block")
async def get_schedule_block(
    ...
    try:
        ...
        result = supabase.table("schedule_blocks").select("*").eq("id", str(block_id)).eq("user_id", current_user["id"]).execute()
...
@router.put("/{block_id}", response_model=ScheduleBlock, summary="Update a schedule block")
async def update_schedule_block(
    ...
    try:
        ...
        existing = supabase.table("schedule_blocks").select("*").eq("id", str(block_id)).eq("user_id", current_user["id"]).execute()
        ...
        result = supabase.table("schedule_blocks").update(update_data).eq("id", str(block_id)).eq("user_id", current_user["id"]).execute()
...
async def delete_schedule_block(
    ...
    try:
        ...
        result = supabase.table("schedule_blocks").delete().eq("id", str(block_id)).eq("user_id", current_user["id"]).execute()
...
@router.post("/{block_id}/reschedule", response_model=ScheduleBlock, summary="Reschedule a block")
...
async def get_schedule_stats(
    ...
    try:
        ...
        supabase = get_supabase_client()
        result = supabase.table("schedule_blocks").select("*").eq("user_id", current_user["id"]).gte("start_time", start_date.isoformat()).lte("end_time", end_date.isoformat()).execute()
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch schedule statistics") 