...
async def log_habit(habit_id: str, user_id: str, mood_before: Optional[str] = None, notes: Optional[str] = None):
    supabase = get_supabase_client()
    log_data = {
        "habit_id": habit_id,
        "user_id": user_id,
...
async def habit_streaks(user_id: str):
    ...
    logs = supabase.table("habit_logs").select("habit_id, completed_at").eq("user_id", user_id).order("completed_at").execute().data
    ...
    return {"user_id": user_id, "streaks": [{"habit_id": k, "streak": v} for k, v in streaks.items()]}

@router.get("/calendar/{user_id}", summary="Return heatmap view of completions")
async def habit_calendar(user_id: str):
    supabase = get_supabase_client()
    logs = supabase.table("habit_logs").select("habit_id, completed_at").eq("user_id", user_id).execute().data
    completions = [
