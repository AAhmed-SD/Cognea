...
@router.get("/dashboard", summary="Personalized dashboard (habits, mood, focus time, goals)")
async def analytics_dashboard(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        schedule_result = supabase.table("schedule_blocks").select("*").eq("user_id", user_id).execute()
        ...
        for i in range(7):
            ...
            day_blocks = [block for block in schedule_blocks if block.get("start_time", "").startswith(date)]
            total_minutes = sum(
                (datetime.fromisoformat(block["end_time"].replace("Z", "+00:00")) - 
                 datetime.fromisoformat(block["start_time"].replace("Z", "+00:00"))).total_seconds() / 60
        ...
        dashboard = {
            "habits": [goal["title"] for goal in goals if goal.get("category") == "habit"],
            ...
            "productivity_score": calculate_productivity_score(tasks, schedule_blocks, goals)
        }
...
@router.get("/trends", summary="Visual insights over time (calendar heatmap, bar charts)")
async def trends(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        tasks_result = supabase.table("tasks").select("*").eq("user_id", user_id).gte("created_at", start_date.isoformat()).execute()
        tasks = tasks_result.data or []
        ...
        for i in range(30):
            ...
            day_tasks = [task for task in tasks if task.get("created_at", "").startswith(date)]
            completed_tasks = [task for task in day_tasks if task.get("status") == "completed"]
...
async def weekly_review(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        completed_tasks = [task for task in tasks if task.get("status") == "completed"]
        missed_tasks = [task for task in tasks if task.get("status") == "pending" and task.get("due_date") and datetime.fromisoformat(task["due_date"].replace("Z", "+00:00")) < end_date]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to load weekly review: {str(e)}")

@router.get("/productivity-patterns", summary="Best day/time insights")
async def productivity_patterns(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to load productivity patterns: {str(e)}")
...
def analyze_productivity_patterns(schedule_blocks):
    ...
    best_day = max(day_counts.items(), key=lambda x: x[1])[0] if day_counts else "Monday"
    ...
    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
        day_blocks = [block for block in schedule_blocks if block.get("start_time", "").startswith(day)]
        total_minutes = sum(
            (datetime.fromisoformat(block["end_time"].replace("Z", "+00:00")) - 
             datetime.fromisoformat(block["start_time"].replace("Z", "+00:00"))).total_seconds() / 60
            for block in day_blocks
