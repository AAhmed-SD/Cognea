...
# CRUD Operations
@router.post(
    "/", response_model=NotificationResponse, summary="Create a new notification"
)
async def create_notification(
    notification: NotificationCreate, current_user: dict = Depends(get_current_user)
):
...
async def get_upcoming_notifications(
    ...
    try:
        ...
        result = (
            ...
            .select("*")
            .eq("user_id", current_user["id"])
            .gte("send_time", now.isoformat())
            .lte("send_time", future_time.isoformat())
            .order("send_time")
            .execute()
        )
