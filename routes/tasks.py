...
async def get_task(
    ...
    try:
        ...
        result = supabase.table("tasks").select("*").eq("id", str(task_id)).eq("user_id", current_user["id"]).execute()
...
async def update_task(
    ...
    try:
        ...
        existing = supabase.table("tasks").select("*").eq("id", str(task_id)).eq("user_id", current_user["id"]).execute()
        ...
        result = supabase.table("tasks").update(update_data).eq("id", str(task_id)).eq("user_id", current_user["id"]).execute()
...
async def delete_task(
    ...
    try:
        ...
        result = supabase.table("tasks").delete().eq("id", str(task_id)).eq("user_id", current_user["id"]).execute()
...
@router.post("/{task_id}/complete", response_model=Task, summary="Mark task as completed")
