...
async def get_flashcards(
    ...
    limit: int = Query(100, ge=1, le=1000, description="Number of flashcards to return"),
    ...
    try:
        ...
        query = supabase.table("flashcards").select("*").eq("user_id", current_user["id"])
        ...
        if tags:
            flashcards = [card for card in flashcards if any(tag in card.tags for tag in tags)]
...
@router.get("/{flashcard_id}", response_model=Flashcard, summary="Get a specific flashcard")
async def get_flashcard(
    ...
    try:
        ...
        result = supabase.table("flashcards").select("*").eq("id", str(flashcard_id)).eq("user_id", current_user["id"]).execute()
...
async def update_flashcard(
    ...
    try:
        ...
        existing = supabase.table("flashcards").select("*").eq("id", str(flashcard_id)).eq("user_id", current_user["id"]).execute()
        ...
        if flashcard_update.last_reviewed_at is not None:
            update_data["last_reviewed_at"] = flashcard_update.last_reviewed_at.isoformat()
        if flashcard_update.next_review_date is not None:
            update_data["next_review_date"] = flashcard_update.next_review_date.isoformat()
        ...
        result = supabase.table("flashcards").update(update_data).eq("id", str(flashcard_id)).eq("user_id", current_user["id"]).execute()
...
async def delete_flashcard(
    ...
    try:
        ...
        result = supabase.table("flashcards").delete().eq("id", str(flashcard_id)).eq("user_id", current_user["id"]).execute()
...
@router.post("/{flashcard_id}/review", response_model=Flashcard, summary="Review a flashcard")
async def review_flashcard(
    flashcard_id: UUID,
    quality: int = Query(..., ge=0, le=5, description="Review quality (0-5, where 5 is perfect)"),
    current_user: dict = Depends(get_current_user)
):
...
@router.get("/due/review", response_model=List[Flashcard], summary="Get flashcards due for review")
async def get_due_flashcards(
    ...
    try:
        ...
        result = supabase.table("flashcards").select("*").eq("user_id", current_user["id"]).lte("next_review_date", datetime.now().isoformat()).order("next_review_date", desc=False).limit(limit).execute()
...
async def get_flashcard_decks(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        result = supabase.table("flashcards").select("deck_id, deck_name").eq("user_id", current_user["id"]).not_.is_("deck_id", "null").execute()
...
async def get_flashcard_stats(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        result = supabase.table("flashcards").select("*").eq("user_id", current_user["id"]).execute()
        ...
        due_cards = len([c for c in cards if c.get("next_review_date") and datetime.fromisoformat(c["next_review_date"].replace("Z", "+00:00")) <= datetime.now()])
        ...
        average_ease_factor = sum(ease_factors) / len(ease_factors) if ease_factors else 0
        ...
        for card in cards:
            if card.get("last_reviewed_at"):
                review_date = datetime.fromisoformat(card["last_reviewed_at"].replace("Z", "+00:00")).date()
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch flashcard statistics") 