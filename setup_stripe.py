...
def create_products():
    ...
    for plan_key, product_data in products.items():
        try:
            ...
            if existing_product:
                print(
                    f"✅ Product '{product_data['name']}' already exists (ID: {existing_product.id})"
                )
    ...
    for plan_key, price_amount in prices.items():
        ...
        try:
            ...
            existing_prices = stripe.Price.list(
                product=products[plan_key].id, active=True, limit=100
            )

            if existing_prices.data:
                print(
                    f"✅ Price for '{plan_key}' already exists (ID: {existing_prices.data[0].id})"
                )
                ...
                print(
                    f"✅ Created price for '{plan_key}' (ID: {price.id}) - ${price_amount/100}/month"
                )
...
def create_webhook():
    ...
    if webhook_url == "https://yourdomain.com/api/stripe/webhook":
        ...
        print(
            "   Example: STRIPE_WEBHOOK_URL=https://yourdomain.com/api/stripe/webhook"
        )
        return None
    ...
    try:
        ...
        if existing_webhook:
            ...
            print(
                "   ⚠️  Webhook secret generated. Please retrieve it securely from the Stripe dashboard."
            )
...
def main():
    ...
    # Create webhook
    webhook = create_webhook()  # noqa: F841
    print()

    # Configure customer portal
    portal = create_customer_portal()  # noqa: F841
    print()
