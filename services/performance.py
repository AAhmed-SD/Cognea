...
class PerformanceMonitor:
    ...
    def record_request(self, endpoint: str, method: str, response_time: float, status_code: int = 200):
    ...
    def get_slowest_endpoints(self, limit: int = 10) -> List[Dict]:
        ...
        return sorted(endpoint_stats, key=lambda x: x["avg_response_time"], reverse=True)[:limit]
...
class DatabaseQueryOptimizer:
    ...
    @staticmethod
    def optimize_select_query(table: str, columns: List[str], filters: Dict = None, limit: int = None) -> str:
        ...
        if scope["type"] == "http":
            ...
            async def custom_send(message):
                if message["type"] == "http.response.start":
                    ...
                    performance_monitor.record_request(path, method, response_time, status)
