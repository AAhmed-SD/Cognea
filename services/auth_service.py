...
from models.auth import (
    UserRole, Permission, UserCreate, UserLogin, UserUpdate, 
    PasswordChange, PasswordResetRequest, PasswordResetConfirm,
    EmailVerificationRequest, EmailVerificationConfirm,
    UserResponse, TokenResponse, RefreshTokenRequest,
    PermissionCheck, RoleAssignment, AuditLogEntry,
    get_user_permissions, has_permission, can_manage_role
)
...
class AuthService:
    ...
    async def refresh_token(self, refresh_token: str) -> Dict[str, str]:
        ...
        try:
            ...
            result = self.supabase.table("users").select("id, role, is_active").eq("id", user_id).execute()
    ...
    async def request_password_reset(self, reset_data: PasswordResetRequest) -> bool:
        ...
        try:
            # Find user by email
            result = self.supabase.table("users").select("id, email, first_name").eq("email", reset_data.email).execute()
    ...
    async def request_email_verification(self, verification_data: EmailVerificationRequest) -> bool:
        """Request email verification"""
        try:
            # Find user by email
            result = self.supabase.table("users").select("id, email, first_name, is_email_verified").eq("email", verification_data.email).execute()
    ...
    async def update_user(self, user_id: str, update_data: UserUpdate) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = self.supabase.table("users").update(update_fields).eq("id", user_id).execute()
    ...
    async def assign_role(self, user_id: str, new_role: UserRole, assigned_by: str, reason: str = None) -> bool:
    ...
    async def get_users(self, current_user_id: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    async def deactivate_user(self, user_id: str, deactivated_by: str, reason: str = None) -> bool:
