...
class StripeService:
    ...
    async def create_portal_session(self, user_id: str) -> stripe.billing_portal.Session:
    ...
    async def get_subscription_status(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if subscription_id:
                try:
                    subscription = await self.redis_client.safe_call("stripe", stripe.Subscription.retrieve, subscription_id)
                    ...
                    logger.warning(f"Failed to retrieve subscription {subscription_id}: {str(e)}")
    ...
    async def _handle_subscription_created(self, subscription: stripe.Subscription):
        ...
        try:
            ...
            user_result = self.supabase.table("users").select("id").eq("stripe_customer_id", customer_id).execute()
            if not user_result.data:
