...
class ContextManager:
    ...
    async def build_user_context(
        ...
        try:
            ...
            await cache_service.set(
                cache_key, context.to_dict(), expire=self.context_cache_ttl
            )
    ...
    async def _fetch_goals(self, user_id: str) -> List[Dict]:
        ...
        try:
            response = (
                await supabase_client.table("goals")
                .select("*")
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def _fetch_habits(self, user_id: str) -> List[Dict]:
        ...
        try:
            response = (
                await supabase_client.table("habits")
                .select("*")
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def _fetch_schedule_blocks(self, user_id: str) -> List[Dict]:
        ...
        try:
            response = (
                await supabase_client.table("schedule_blocks")
                .select("*")
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def _fetch_flashcards(self, user_id: str) -> List[Dict]:
        ...
        try:
            response = (
                await supabase_client.table("flashcards")
                .select("*")
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def _fetch_productivity_data(self, user_id: str) -> Dict:
        ...
        try:
            ...
            completed_with_time = [
                t for t in tasks if t.get("completed_at") and t.get("created_at")
            ]
            ...
            logger.error(
                f"Error fetching productivity data for user {user_id}: {str(e)}"
            )
            return {}
    ...
    async def _fetch_user_preferences(self, user_id: str) -> Dict:
        ...
        try:
            ...
            preferences = {
                "work_hours": settings.get(
                    "work_hours", {"start": "09:00", "end": "17:00"}
                ),
                "timezone": settings.get("timezone", "UTC"),
                "notification_preferences": settings.get(
            ...
            logger.error(
                f"Error fetching user preferences for user {user_id}: {str(e)}"
            )
            return {}
    ...
    async def _fetch_conversation_history(self, user_id: str) -> List[Dict]:
        ...
        try:
            response = (
                await supabase_client.table("ai_conversations")
    ...
    async def add_conversation_message(
        self, user_id: str, role: str, content: str, metadata: Optional[Dict] = None
    ):
        ...
        try:
            ...
            if response.data:
                ...
                conv_data["messages"] = conv_data["messages"][
                    -self.conversation_memory_size :
                ]
            ...
            logger.error(
                f"Error adding conversation message for user {user_id}: {str(e)}"
            )
    ...
    async def get_context_summary(self, user_id: str) -> Dict:
        ...
        summary = {
            ...
            "task_summary": {
                ...
                "pending_tasks": len(
                    [t for t in context.tasks if t.get("status") != "completed"]
                ),
                "completed_tasks": len(
                    [t for t in context.tasks if t.get("status") == "completed"]
                ),
                "high_priority_tasks": len(
                    [
                        ...
                        if t.get("priority") == "high" or t.get("priority") == "urgent"
                    ]
                ),
            },
            "goal_summary": {
                ...
                "active_goals": len(
                    [g for g in context.goals if g.get("status") == "active"]
                ),
                "completed_goals": len(
                    [g for g in context.goals if g.get("status") == "completed"]
                ),
            },
            ...
            "recent_activity": {
                ...
                "upcoming_schedule": [
                    b
                    for b in context.schedule_blocks
                    if b.get("start_time") > datetime.utcnow().isoformat()
                ][:5],
            },
        }
