...
class EnhancedOpenAIService:
    ...
    async def generate_task_from_text(
        self, text: str, user_id: str, user_context: Optional[Dict] = None
    ) -> Dict:
        ...
        messages = [
            ...
            {"role": "user", "content": f"Create a task from this text: {text}"},
        ]
    ...
    async def optimize_schedule(
        ...
        response = await self.chat_completion(
            messages=messages, user_id=user_id, use_functions=True, context=context
        )
    ...
    async def generate_productivity_insights(
        ...
        response = await self.chat_completion(
            messages=messages, user_id=user_id, use_functions=True, context=user_data
        )
