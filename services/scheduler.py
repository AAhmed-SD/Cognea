...
class SimpleScheduler:
    ...
    def calculate_task_score(self, task: Task, time_slot: TimeSlot, user_energy: int = 5) -> float:
        ...
        score = priority_weight * length_factor * availability_factor * energy_factor * time_factor
    ...
    def _calculate_energy_factor(self, task: Task, time_slot: TimeSlot, user_energy: int) -> float:
    ...
    def schedule_tasks(self, tasks: List[Task], time_slots: List[TimeSlot], 
                      user_energy: int = 5) -> List[Dict[str, Any]]:
        """Schedule tasks into time slots using greedy algorithm."""
        
        # Sort tasks by priority first
        sorted_tasks = sorted(tasks, key=lambda t: self.config.priority_weights.get(t.priority, 1.0), reverse=True)
        ...
        for task in sorted_tasks:
            ...
            for i, slot in enumerate(sorted_slots):
                ...
                if score > best_score and task.estimated_minutes <= slot.duration_minutes:
                    best_score = score
                    best_slot = (i, slot)
            ...
            if best_slot:
                ...
                schedule.append({
                    ...
                    "end_time": slot.start_time + timedelta(minutes=task.estimated_minutes)
                })
        
        return schedule
    
    def optimize_schedule(self, current_schedule: List[Dict], user_energy: int = 5) -> List[Dict]:
    ...
    def add_breaks(self, schedule: List[Dict], min_break_minutes: int = None) -> List[Dict]:
            ...
            if i < len(schedule) - 1:
                ...
                if time_between >= min_break_minutes:
                    break_slot = TimeSlot(
                        ...
                        end_time=current_task_end + timedelta(minutes=min_break_minutes),
    ...
    def get_schedule_insights(self, schedule: List[Dict]) -> Dict[str, Any]:
        ...
        total_duration = sum(item["task"].estimated_minutes for item in schedule if item["task"].id != "break")
