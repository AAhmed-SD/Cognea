...
class NotionFlashcardGenerator:
    ...
    async def generate_flashcards_from_page(self, page_id: str, count: int = 5, difficulty: str = "medium") -> List[FlashcardData]:
    ...
    async def generate_flashcards_from_database(self, database_id: str, count: int = 10, difficulty: str = "medium") -> List[FlashcardData]:
        ...
        try:
            ...
            logger.error(f"Failed to generate flashcards from database {database_id}: {e}")
            raise
    
    async def _extract_page_content_from_database_item(self, page_data: Dict[str, Any]) -> str:
        ...
        for prop_name, prop_data in properties.items():
            if prop_data.get("type") == "rich_text":
                text = "".join([text.get("plain_text", "") for text in prop_data.get("rich_text", [])])
                if text:
                    content_parts.append(f"{prop_name}: {text}")
            elif prop_data.get("type") == "title":
                text = "".join([text.get("plain_text", "") for text in prop_data.get("title", [])])
                if text:
                    content_parts.append(f"{prop_name}: {text}")
    ...
    async def _generate_flashcards_with_ai(self, content: str, title: str, count: int, difficulty: str, 
                                         source_page_id: str, source_page_title: str) -> List[FlashcardData]:
        ...
        try:
            ...
            # Parse the response
            flashcards = self._parse_ai_response(response, source_page_id, source_page_title)
    ...
    async def _generate_flashcards_from_multiple_sources(self, content_items: List[Dict[str, str]], count: int, 
                                                       difficulty: str, source_database_id: str, source_database_title: str) -> List[FlashcardData]:
    ...
    def _create_flashcard_prompt(self, content: str, title: str, count: int, difficulty: str) -> str:
...
"""
    ...
    def _parse_ai_response(self, response: str, source_page_id: str, source_page_title: str) -> List[FlashcardData]:
        ...
        for line in lines:
            ...
            try:
                # Parse the flashcard
                flashcard = self._parse_flashcard_line(line, source_page_id, source_page_title)
                if flashcard:
    ...
    def _parse_flashcard_line(self, line: str, source_page_id: str, source_page_title: str) -> Optional[FlashcardData]:
        ...
        try:
            ...
            # Extract difficulty
            difficulty_match = re.search(r'DIFFICULTY:\s*(easy|medium|hard)', line, re.IGNORECASE)
    ...
    async def generate_flashcards_from_text(self, text: str, title: str = "Manual Input", count: int = 5, 
                                          difficulty: str = "medium") -> List[FlashcardData]:
