...
class NotionSyncManager:
    ...
    def __init__(self, notion_client: NotionClient, flashcard_generator: NotionFlashcardGenerator):
        """Initialize the sync manager."""
        self.notion_client = notion_client
        self.flashcard_generator = flashcard_generator
        self.supabase = get_supabase_client()
    
    async def sync_page_to_flashcards(self, user_id: str, notion_page_id: str, 
                                    sync_direction: str = "notion_to_cognie") -> SyncStatus:
        """Sync a Notion page to Cognie flashcards with debounce/echo prevention."""
        try:
            # Get local last_synced_ts
            local_flashcard = self.supabase.table("flashcards").select("last_synced_ts").eq("user_id", user_id).eq("source_page_id", notion_page_id).order("last_synced_ts", desc=True).limit(1).execute()
            ...
            for flashcard in flashcards:
                saved_flashcard = await self._save_flashcard_to_cognie(user_id, flashcard)
                if saved_flashcard:
                    # Update last_synced_ts
                    self.supabase.table("flashcards").update({"last_synced_ts": page.last_edited_time.isoformat()}).eq("id", saved_flashcard["id"]).execute()
            ...
            logger.info(f"Successfully synced {len(saved_flashcards)} flashcards from Notion page {notion_page_id}")
            return sync_status
    ...
    async def sync_database_to_flashcards(self, user_id: str, notion_database_id: str,
                                        sync_direction: str = "notion_to_cognie") -> SyncStatus:
        ...
        try:
            ...
            flashcards = await self.flashcard_generator.generate_flashcards_from_database(
            ...
            logger.info(f"Successfully synced {len(saved_flashcards)} flashcards from Notion database {notion_database_id}")
            return sync_status
    ...
    async def sync_flashcards_to_notion(self, user_id: str, flashcard_ids: List[str], 
                                      target_page_id: str) -> SyncStatus:
        ...
        try:
            ...
            logger.info(f"Successfully synced {len(flashcards)} flashcards to Notion page {target_page_id}")
            return sync_status
    ...
    async def _save_flashcard_to_cognie(self, user_id: str, flashcard: FlashcardData) -> Optional[Dict[str, Any]]:
    ...
    async def _get_flashcards_from_cognie(self, user_id: str, flashcard_ids: List[str]) -> List[Dict[str, Any]]:
        """Get flashcards from Cognie database."""
        try:
            result = self.supabase.table("flashcards").select("*").eq("user_id", user_id).in_("id", flashcard_ids).execute()
            return result.data or []
        except Exception as e:
            logger.error(f"Failed to get flashcards from Cognie: {e}")
            return []
    
    def _create_notion_content_from_flashcards(self, flashcards: List[Dict[str, Any]]) -> str:
    ...
    async def _save_sync_status(self, sync_status: SyncStatus):
        ...
        try:
            ...
            existing = self.supabase.table("notion_sync_status").select("*").eq("user_id", sync_status.user_id).eq("notion_page_id", sync_status.notion_page_id).execute()
            
            if existing.data:
                # Update existing
                self.supabase.table("notion_sync_status").update(sync_data).eq("user_id", sync_status.user_id).eq("notion_page_id", sync_status.notion_page_id).execute()
            else:
                # Insert new
    ...
    async def get_sync_status(self, user_id: str, notion_page_id: str) -> Optional[SyncStatus]:
        """Get sync status for a specific page."""
        try:
            result = self.supabase.table("notion_sync_status").select("*").eq("user_id", user_id).eq("notion_page_id", notion_page_id).execute()
    ...
    async def get_user_sync_history(self, user_id: str) -> List[SyncStatus]:
        ...
        try:
            result = self.supabase.table("notion_sync_status").select("*").eq("user_id", user_id).order("last_sync_time", desc=True).execute()
