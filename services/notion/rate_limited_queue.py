...
class NotionRateLimitedQueue:
    ...
    async def enqueue_request(self, method: str, endpoint: str, data: Optional[Dict] = None, 
                            headers: Optional[Dict] = None, priority: int = 1) -> asyncio.Future:
    ...
    async def _get_highest_priority_request(self) -> tuple[NotionAPIRequest, asyncio.Future]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if request.method == "GET":
                    result = await self.notion_client._make_request("GET", request.endpoint, headers=request.headers)
                elif request.method == "POST":
                    result = await self.notion_client._make_request("POST", request.endpoint, json=request.data, headers=request.headers)
                elif request.method == "PUT":
                    result = await self.notion_client._make_request("PUT", request.endpoint, json=request.data, headers=request.headers)
                elif request.method == "DELETE":
                    result = await self.notion_client._make_request("DELETE", request.endpoint, headers=request.headers)
                else:
