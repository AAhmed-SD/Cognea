...
class CostControlService:
    ...
    async def get_cached_flashcards(self, cache_key: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            result = self.supabase.table("flashcard_cache").select("*").eq("cache_key", cache_key).gte("created_at", cutoff_time.isoformat()).execute()
    ...
    async def cache_flashcards(self, cache_key: str, flashcards: List[Dict]):
        ...
        try:
            ...
            logger.info(f"Cached {len(flashcards)} flashcards with key {cache_key[:8]}...")
            
        except Exception as e:
            logger.error(f"Error caching flashcards: {e}")
    
    async def process_content_for_flashcards(self, content: str, title: str, count: int, difficulty: str) -> Dict[str, Any]:
        ...
        logger.info(f"Processing content: {len(content)} chars -> {len(compressed_content)} chars -> {len(chunks)} chunks")
    ...
    def calculate_cost_savings(self, original_tokens: int, compressed_tokens: int) -> Dict[str, float]:
